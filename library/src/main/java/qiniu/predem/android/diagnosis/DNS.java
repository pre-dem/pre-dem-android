...
public class DNS {
    ...
    private static String getDiagUrl(String url) throws IOException {
        String fetch = AsyncRun.httpGetString(url); //("http://ns.pbt.cachecn.net/fast_tools/fetch_ldns_diag_client.php");
        if (fetch == null) {
            return null;
        }
    ...
    public static String check(String url) {
        ...
        String result = null;
        try {
            result = AsyncRun.httpGetString(f);
        } catch (IOException e) {
            e.printStackTrace();
        }

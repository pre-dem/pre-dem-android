...
public class TcpPing implements Task {
...
//        this.output = output;
    ...
    private void run() {
        ...
        try {
            ...
            AsyncRun.runInMain(new Runnable() {
                @Override
                public void run() {
                    complete.complete(new Result(UnkownHost, "", 0, 0, 0, 0, 0, 0, 0));
                }
            });
            return;
        }
            ...
            try {
                ...
                if (i == 0) {
                    AsyncRun.runInMain(new Runnable() {
                        @Override
                        public void run() {
                            complete.complete(new Result(code2, ip, 0, 0, 0, 0, 0, 1, 1));
                        }
                    });
                    return;
                } else {
        ...
        if (index == -1) {
            complete.complete(new Result(Stopped, ip, 0, 0, 0, 0, 0, 0, 0));
            return;
        }
    ...
    private Result buildResult(int[] times, int index, String ip, int dropped) {
        ...
        return new Result(0, ip, max, min, sum / (index + 1), sum, 0, index + 1, dropped);
    }
    ...
    public static final class Result {
        ...
        public Result(int code, String ip, int maxTime, int minTime, int avgTime, long totalTime,
